...
export const getBeautifulNumber = (value: { toString: () => string }, separator = ' ') =>
...
export const toStringArray = (arr: { value: string; number: number }[]) =>
  arr.map(({ value, number }) => `${value}_${number}`);

export const transformCustomers = (
  customers: { id: string; name: string; age: number; isSubscribed: boolean }[]
): Record<string, { name: string; age: number; isSubscribed: boolean }> => {
  return customers.reduce((acc, customer) => {
    acc[customer.id] = { name: customer.name, age: customer.age, isSubscribed: customer.isSubscribed };
    return acc;
  }, {} as Record<string, { name: string; age: number; isSubscribed: boolean }>);
};
